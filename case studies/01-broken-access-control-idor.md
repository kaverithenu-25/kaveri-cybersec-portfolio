...
The vulnerability was assessed across the standard CIA dimensions:    

- **Confidentiality** — High. Direct exposure of another user's financial and personal data, including transaction history and account-linked information.

- **Integrity** — Medium to High. If the same endpoint also accepts POST or PUT requests, the attacker can modify or delete records belonging to other users, not just read them.

- **Availability** — Low. Unlikely to cause direct service disruption, though deletion variants of the same flaw can have downstream effects.

- **CVSS 3.1** base score — 7.7 (High). Vector: AV:N/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:N.

- **Business impact** — Critical. Regulatory exposure under data protection law, reputational damage, contractual breach, and potential audit failure.
